...
class PIMBenchTestCase
{
  public:
        ...
        : kernel_type_(k), batch_(b), out_(out), in_(in)
    {
        ...
        if (kernel_type_ == KernelType::JOIN) dim_data_ = new DataDim(kernel_type_, batch_, out_, in_, in_, true);
        else dim_data_ = new DataDim(kernel_type_, batch_, out_, in_, false);
    }
    ...
    {
        std::cout << "run(shared_ptr<MultiChannelMemorySystem> mem_, uint64_t *cycle) @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@" << std::endl;
...
///////////////////////////////////////////////////////////////////////////////
class JoinPIMBenchTest : public PIMBenchTestCase
{
  public:
    JoinPIMBenchTest(KernelType k, unsigned b, unsigned out, unsigned in)
    ...
    uint64_t measureCycle(bool is_pim_ = false)
    {
        ...
        std::cout << "measureCycle @@@@@@@@@@@@@@@@@@@@@@@@" << std::endl;
        ...
        if (is_pim_ == true)
        {
            ...
            kernel_->executeJoinBench(dim_data_->input_npbst_.getTotalDim(), dim_data_->input1_npbst_.getTotalDim(),
                                        pimBankType::ALL_BANK, kernel_type_, input_row0_, input_row1_);

            std::cout << "PIM 1 @@@@@@@@@@@@@@@@@@@@@@@@" << std::endl;

            kernel_->runPIM();
            std::cout << "PIM 2 @@@@@@@@@@@@@@@@@@@@@@@@" << std::endl;

            cycle = kernel_->getCycle();
            std::cout << "pim cycle : " << cycle << std::endl;

        }
        else
        {
            std::cout << "NOT PIM @@@@@@@@@@@@@@@@@@@@@@@@" << std::endl;
            uint32_t input_data_size_in_byte = dim_data_->input_dim_ * 8;
            // std::cout << "getDataSize : " << dim_data_->getDataSize(dim_data_->input_dim_, dim_data_->batch_size_) << std::endl;
            // std::cout << "input_data_size_in_byte : " << input_data_size_in_byte << std::endl;
            uint32_t input1_data_size_in_byte = dim_data_->input_dim_ * 8;
            uint32_t output_data_size_in_byte = dim_data_->output_dim_ * 8;

            /////// first partition
            starting_addr = genMemTraffic(mem_, false, input_data_size_in_byte, starting_addr);
            genMemTraffic(mem_, true, input_data_size_in_byte, starting_addr);

            genMemTraffic(mem_, false, input1_data_size_in_byte, starting_addr);
            genMemTraffic(mem_, true, input1_data_size_in_byte, starting_addr);
            ...
            run(mem_, &cycle);
            std::cout << "normal cycle : " << cycle << std::endl;
            // cycle += 900;
        }
        return cycle;
    }
...
class PIMBenchFixture : public testing::Test
{
  public:
    ...
    void setPIMBenchTestCase(KernelType k, unsigned out, unsigned in, unsigned batch = 1)
    {
        ...
        {
            perfTest = new JoinPIMBenchTest(k, batch, out, in);
        }
        else
        {
            throw invalid_argument("Invalid kernel type");
    ...
    {
        std::cout << "perfTest->printTestMessage(false) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
        perfTest->printTestMessage(false);
        std::cout << "non_pim_cycle_ = perfTest->measureCycle(false); !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
        non_pim_cycle_ = perfTest->measureCycle(false);
        std::cout << " printStats(non_pim_cycle_) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << std::endl;
