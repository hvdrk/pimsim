...
void PIMCmd::fromInt(uint32_t val)
{
    ...
    switch (type_)
    {
        ...
        case PIMCmdType::STS:
            dst_ = PIMOpdType(fromBit(val, 3, 25));
            src0_ = PIMOpdType(fromBit(val, 3, 22));
            isAuto_ = fromBit(val, 1, 21);
            dstIdx_ = fromBit(val, 4, 17);
            dstIdx1_ = fromBit(val, 6, 11);
            src0Idx_ = fromBit(val, 4, 6);
            src1Idx_ = fromBit(val, 6, 0);
            break;
        default:
            break;
    }
...
uint32_t PIMCmd::toInt() const
{
    ...
    switch (type_)
    {
        ...
        case PIMCmdType::STS:
            val |= toBit(int(dst_), 3, 25);
            val |= toBit(int(src0_), 3, 22);
            val |= toBit(isAuto_, 1, 21);
            val |= toBit(dstIdx_, 4, 17);
            val |= toBit(dstIdx1_, 6, 11);
            val |= toBit(src0Idx_, 4, 6);
            val |= toBit(src1Idx_, 6, 0);
            break;
